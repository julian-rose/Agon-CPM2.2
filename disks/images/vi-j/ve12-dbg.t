...
! BEGIN CONSOLE DEFS
tty.init() ;  	! jhr, the embedded startup code found in t3x9-src.t
tty.mode(raw) ;
tty.fini() ;
...
tty.clreol() tty.writes("\e[0K");

tty.clear()	tty.writes("\e[2J");

tty.move(x, y) do var lx, ly;
	lx := x + 1;
	ly := y + 1;
	tty.writec('\e');
	tty.writec('[');
	tty.writec((ly / 10)+'0');
	tty.writec((ly mod 10)+'0');
	tty.writec(';');
	tty.writec((lx / 10)+'0');
	tty.writec((lx mod 10)+'0');
	tty.writec('H');
end
...
postnum(n) do var i;
	...
	i := 0;
	while( 10000 <= n ) do i := i + 1; n := n - 10000; end
	tty.writec(i+'0'); i := 0;
	while( 1000 <= n ) do i := i + 1; n := n - 1000; end
	tty.writec(i+'0'); i := 0;
	while( 100 <= n ) do i := i + 1; n := n - 100; end
	tty.writec(i+'0'); i:= 0;
	while( 10 <= n ) do i := i + 1; n := n - 10; end
	tty.writec(i+'0'); i:=0;
	tty.writec(n+'0');
        tty.writec(' ');
end
...
delay();
		if (Buffer::pos(i) = '\n') do
post("killblank newline at pos ");
postnum(i);
delay();
			leave;
		end
		if (\iswhite(Buffer::pos(i))) do
post("killblank !iswhite at pos ");
postnum(i);
delay();
			goto(p);
postd("killblank here 5.2");
			return;
		end
	end
postd("jhr, killblank here 2");
	mark();
	eol();
	setblock();
        p := p - Eblock + Block;
...
delay();
	n := 0;
	while (1) do
postd("insmode, here 1");
		killblank();
postd("insmode, here 1.1");
		sync();
		ie (open) do
			k := '\r';
			Iobuf::0 := 0;
...
delay();
		if (ks /\ inserts(Iobuf)) do
			nospace();
postd("insmode nospace");
			leave;
		end
		for (i=0, ks) right();
		if (k \= '\r') do postd("leave"); leave; end
		n := n+1;
		if (inserts("\n")) do
postd("insmode nospace 2");
			nospace();
			leave;
		end
		displine();
